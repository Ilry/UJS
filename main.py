...
for user in settings.user_info:
    ...
    for _ in range(10):  # 尝试登录
        wd.get(settings.url)
        captchaImg = wd.find_element_by_xpath(
            '//form[@id="casLoginForm"]//img[@id="captchaImg"]')
        ver_code = get_vercode(wd, captchaImg)
        if len(ver_code) != 4:
            continue

        username_input = wd.find_element_by_xpath(
            '//form[@id="casLoginForm"]//input[@id="username"]')
        password_input = wd.find_element_by_xpath(
            '//form[@id="casLoginForm"]//input[@id="password"]')
        captcha_input = wd.find_element_by_xpath(
            '//form[@id="casLoginForm"]//input[@id="captchaResponse"]')
        login_button = wd.find_element_by_xpath(
            '//form[@id="casLoginForm"]//button[@type="submit"]')
    ...
    while wd.find_elements_by_xpath('//div[@class="weui_media_bd"]') \
        ...
        wd.find_element_by_xpath(
            '//a[@class="weui_btn  weui_btn_primary"]').click()
    ...
    # 其他异常
    other_symptoms_select = Select(
        wd.find_element_by_xpath('//select[@id="qtyc"]'))
...
with open('/Python/'+result_path, 'w') as f:
    json.dump(settings.result_text, f)

# 发送邮件，通知用户
result_path='/Python/'+result_path
